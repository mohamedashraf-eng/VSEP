...
ifeq ($(rebuild),1)
	...
	cd $(BUILD_DIR)/ && \
	cmake -S $(CURDIR) -B $(BUILD_DIR) -DSOFTWARE_COMPONENT=bootloader_swc $(CMAKE_BUILD_VARIABLES) ;\
 	make bootloader_swc install -j$(MAXIMUM_CPU_CORES)
...
ifeq ($(rebuild),1)
	...
	@echo "BUILD_DIR=$(BUILD_DIR)"
	cd $(BUILD_DIR)/ && \
	cmake -S $(CURDIR) -B $(BUILD_DIR) -DSOFTWARE_COMPONENT=target_app_swc $(CMAKE_BUILD_VARIABLES) ;\
 	make target_app_swc install -j$(MAXIMUM_CPU_CORES)
...
all: build_bootloader build_target_app

.PHONY: build_bootloader build_target_app unit_test test all check_dependencies coverage